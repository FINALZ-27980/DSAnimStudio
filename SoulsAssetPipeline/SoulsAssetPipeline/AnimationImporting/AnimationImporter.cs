...
namespace SoulsAssetPipeline.AnimationImporting
{
    public static class AnimationImporter
    {
        public class AnimationImportSettings
        {
            ...
            public bool UseRootMotionScaleOverride;
            public bool ConvertFromZUp; // Not needed for anims? need to see if it's just my test fbx
            public Dictionary<string, NewBlendableTransform> ExistingBoneDefaults;
            public HavokAnimationData ExistingHavokAnimationTemplate;
            ...
            public bool FlipQuaternionHandedness;

            public bool NegateQuaternionX = true;
            public bool NegateQuaternionY = true;
            public bool NegateQuaternionZ = true;
            public bool NegateQuaternionW;

            public List<string> BonesToFlipBackwardsAboutYAxis = new List<string>();

            public bool InitalizeUnanimatedTracksToTPose;
        }

        public static ImportedAnimation ImportFBX(string fbxPath, AnimationImportSettings settings)
        {
            using AssimpContext context = new AssimpContext();
            return ImportFromAssimpScene(context.ImportFile(fbxPath, PostProcessSteps.CalculateTangentSpace | PostProcessSteps.OptimizeGraph | PostProcessSteps.GlobalScale), settings);
        }
            ...
            AnimationImportSettings settings)
        {
            ...
            foreach (var anim in scene.Animations)
            {
                ...
                {
                    // Setup framerate.
                    _ = (settings.ResampleToFramerate / anim.TicksPerSecond);
                    ...
                    (from n in anim.NodeAnimationChannels select n.NodeName into n orderby n select n).ToList();
                    ...
                    for (int i = 0; i < anim.NodeAnimationChannelCount; i++)
                    {
                        ...
                        if (isRootMotionNode)
                        {
                            ...
                            if (hasRotation && settings.EnableRotationalRootMotion)
                            {
                                ...
                                {
                                    int frame = (int)Math.Floor(rotationKey.Time * resampleTickMult);

                                    var curFrameRotation = rotationKey.Value.ToNumerics();
                                    ...
                                    {
                                        float lerpS = 1f * (f - lastKeyIndex) / (frame - lastKeyIndex);
                                        var quaternion = rootMotionRotationFrames[lastKeyIndex];
                                        var blendTo = curFrameRotation;

                                        var blended = NQuaternion.Lerp(quaternion, blendTo, lerpS);
                            ...
                            if (transformIndex >= 0 && transformIndex < transformTrackNames.Count)
                            {
                                ...
                                if (hasPosition)
                                {
                                    ...
                                    foreach (var keyPos in nodeChannel.PositionKeys)
                                    {
                                        ...
                                        {
                                            float lerpS = 1f * (f - lastKeyIndex) / (frame - lastKeyIndex);
                                            var translation = result.Frames[lastKeyIndex].BoneTransforms[transformIndex].Translation;
                                            var blendTo = curFrameTransform.Translation;

                                            var blended = NVector3.Lerp(translation, blendTo, lerpS);
                                ...
                                if (hasScale)
                                {
                                    ...
                                    foreach (var keyPos in nodeChannel.ScalingKeys)
                                    {
                                        ...
                                        {
                                            float lerpS = 1f * (f - lastKeyIndex) / (frame - lastKeyIndex);
                                            var scale = result.Frames[lastKeyIndex].BoneTransforms[transformIndex].Scale;
                                            var blendTo = curFrameTransform.Scale;

                                            var blended = NVector3.Lerp(scale, blendTo, lerpS);
                                ...
                                if (hasRotation)
                                {
                                    ...
                                    foreach (var keyPos in nodeChannel.RotationKeys)
                                    {
                                        ...
                                        {
                                            float lerpS = 1f * (f - lastKeyIndex) / (frame - lastKeyIndex);
                                            var rotation = result.Frames[lastKeyIndex].BoneTransforms[transformIndex].Rotation;
                                            var blendTo = curFrameTransform.Rotation;

                                            var blended = NQuaternion.Slerp(rotation, blendTo, lerpS);
                                            //blended = NQuaternion.Normalize(blended);
                    ...
                    for (int f = 0; f < result.Frames.Count; f++)
                    {
                        if (f > 0 && settings.EnableRotationalRootMotion)
                        {
                            ...
                            if (NMatrix.Invert(NMatrix.CreateFromQuaternion(rootMotionRotationFrames[f - 1]), out var inverseOldMat))
                            {
