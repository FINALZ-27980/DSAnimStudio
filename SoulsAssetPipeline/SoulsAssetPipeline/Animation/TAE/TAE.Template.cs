    ...
    public partial class TAE
    {
        ...
        public class Template : Dictionary<long, Template.BankTemplate>
        {
                ...
                : base()
            {
                ...
                foreach (XmlNode item in templateNode.SelectNodes("bank"))
                {
                    var newBank = new BankTemplate(item, out long basedOn);
            ...
            public static Template ReadXMLFile(string path)
            {
                ...
                return new Template(xmlDocument);
            }
            ...
            public class ParameterTemplate
            {
                ...
                public object StringToValue(string str)
                {
                    ...
                    switch (Type)
                    {
                        ...
                        case ParamType.f32grad:
                            var array = str.Split('|');
                            float gradStart = float.Parse(array[0]);
                            float gradEnd = float.Parse(array[1]);
				...
				{
				case ParamType.aob:
					return string.Join(" ", ((byte[])val).Select((byte b) => b.ToString("X2")));
				case ParamType.x8:
					return ((byte)val).ToString("X2");
				case ParamType.x16:
					return ((ushort)val).ToString("X4");
				case ParamType.x32:
					return ((uint)val).ToString("X8");
				case ParamType.x64:
					return ((ulong)val).ToString("X16");
				case ParamType.b:
					if (!(bool)val)
					{
						return "False";
                ...
                internal object ReadValue(BinaryReaderEx br)
                {
                    switch (Type)
                    {
                        ...
                        case ParamType.f32grad:
                            ...
                            float gradEnd = br.ReadSingle();
                            return new System.Numerics.Vector2(x, gradEnd);
                ...
                internal void AssertValue(BinaryReaderEx br)
                {
                    switch (Type)
                    {
                        ...
                        case ParamType.aob:
                            ...
                            break;
                        case ParamType.b: br.Position++; break;
                        case ParamType.u8: br.Position++; break;
                        case ParamType.s8: br.Position++; break;
                        case ParamType.u16: br.Position += 2L; break;
                        case ParamType.s16: br.Position += 2L; break;
                        case ParamType.u32: br.Position += 4L; break;
                        case ParamType.s32: br.Position += 4L; break;
                        case ParamType.u64: case ParamType.x64: br.Position += 8L; break;
                        case ParamType.s64: br.Position += 8L; break;
                        case ParamType.f32: br.Position += 4L; break;
                        case ParamType.f32grad:
                            br.Position += 8L;
                            break;
                        case ParamType.f64: br.Position += 8L; break;
#else
                        case ParamType.aob:
                            var assertAob = (byte[])ValueToAssert;
                            br.Position += assertAob.Length;
                ...
                internal ParameterTemplate(long bankId, long eventId, long paramIndex, XmlNode paramNode, int offset)
                {
                    ...
                    if (paramNode.SelectNodes("entry").Count > 0)
                    {
                        EnumEntries = new Dictionary<string, object>();
