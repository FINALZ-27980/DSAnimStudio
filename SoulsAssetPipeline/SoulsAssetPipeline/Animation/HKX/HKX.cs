...
#pragma warning disable CS1591
        ...
        public static HKX Read(string path, HKXVariation variation, bool isDS1RAnimHotfix, bool deserializeObjects = true)
        {
            using (FileStream stream = File.OpenRead(path))
            {
                ...
                HKX hKX = new HKX();
                hKX.IsDS1RAnimHotfix = isDS1RAnimHotfix;
                hKX.Variation = variation;
                hKX.DeserializeObjects = deserializeObjects;
                br = SFUtil.GetDecompressedBR(br, out DCX.Type fileCompression);
                hKX.Compression = fileCompression;
                hKX.Read(br);
                return hKX;
            }
        }
        ...
        public class HKXSection
        {
            ...
            public void WriteData(BinaryWriterEx bw, HKX hkx, HKXVariation variation)
            {
                ...
                {
                    @object.Write(hkx, this, bw, absoluteOffset, variation);
                }
                ...
                {
                    localReference.Write(bw);
                }
                while ((bw.Position % 16) != 0L)
                {
                ...
                foreach (var virtualReference in VirtualReferences)
                {
                    virtualReference.Write(bw);
                }
                while ((bw.Position % 16) != 0L)
                {
                    bw.WriteByte(0xFF); // 16 byte align
        ...
        public class HKXLocalReference
        {
            ...
            public HKXObject SourceObject;

            // The destination object linked
            public HKXObject DestObject;
        ...
        public abstract class HKXObject
        {
            ...
            internal void ResolveDestinations(HKX hkx, HKXSection section)
            {
                ...
                foreach (LocalFixup dest in FindLocalFixupDestinations(section))
                {
                ...
                foreach (GlobalFixup dest in FindGlobalFixupDestinations(hkx, section))
                {
                    if (dest.Reference == null)
                    {
                        var reference = new HKXGlobalReference();
                    ...
                    {
                        var reference2 = dest.Reference;
                        reference2.DestSection = section;
                        reference2.DestLocalOffset = dest.Dst - SectionOffset;
                        reference2.DestObject = this;
                    }
                }
            }
        }
        ...
        public class HKArrayData<T> : HKXObject where T : IHKXSerializable, new()
        {
            ...
            public override void Write(HKX hkx, HKXSection section, BinaryWriterEx bw, uint sectionBaseOffset, HKXVariation variation)
            {
                ...
                {
                    element.Write(hkx, section, bw, sectionBaseOffset, variation);
                }
                while ((bw.Position % 16) != 0L)
                {
                ...
                {
                    element2.WriteReferenceData(hkx, section, bw, sectionBaseOffset, variation);
                }
        ...
        public class HKXGenericObject : HKXObject
        {
            ...
            public void Read(HKX hkx, HKXSection section, BinaryReaderEx br, uint size, HKXVariation variation)
            {
                ...
                DataSize = size;
                List<LocalFixup> list = FindLocalFixupSources(section);
                ...
                foreach (var src in list)
                {
                    ...
                    {
                        var reference2 = src.Reference;
                        reference2.SourceLocalOffset = src.Src - SectionOffset;
                        reference2.SourceObject = this;
                    }
                    LocalReferences.Add(src.Reference);
                }

                // Resolve references where this object is a global source
                List<GlobalFixup> list2 = FindGlobalFixupSources(section);
                GlobalReferences = new List<HKXGlobalReference>();
                foreach (GlobalFixup src in list2)
                {
                    if (src.Reference == null)
                    {
                        var reference = new HKXGlobalReference();
        ...
        public class HKXClassNames : HKXObject
        {
            ...
            public override void Write(HKX hkx, HKXSection section, BinaryWriterEx bw, uint sectionBaseOffset, HKXVariation variation)
            {
                ...
                {
                    className.Write(bw, sectionBaseOffset);
                }
                while ((bw.Position % 16) != 0L)
                {
                    // Write padding bytes to 16 byte align
