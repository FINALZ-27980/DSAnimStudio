            ...
            lock (_lock_GenFakeFromTagFile)
            {
                ...
                if (obj.Type.Name == "hkRootLevelContainer")
                {
                    void DoVariant (IHkObject mainVariant)
                    {
                        ...
                        {
                            foreach (var item2 in skeletons.Value as IReadOnlyList<IHkObject>)
                            {
                                var skel = item2.Value as HkClass;
                                if (skel == null)
                                    continue;
                                ...
                                foreach (var kvp in skel.Value as IReadOnlyDictionary<HkField, IHkObject>)
                                {
                                    ...
                                    else if (kvp.Key.Name == "bones")
                                    {
                                        ...
                                        foreach (var item3 in kvp.Value.Value as IReadOnlyList<IHkObject>)
                                        {
                                            var boneName = SelectFieldFromClass(item3.Value as IReadOnlyDictionary<HkField, IHkObject>, "name") as HkString;
                                    ...
                                    else if (kvp.Key.Name == "referencePose")
                                    {
                                        ...
                                        foreach (var item4 in kvp.Value.Value as IReadOnlyList<IHkObject>)
                                        {
                                            var translation = ((SelectFieldFromClass(item4.Value as IReadOnlyDictionary<HkField, IHkObject>, "translation") as IHkObject).Value as IReadOnlyList<IHkObject>).Cast<HkSingle>().ToList();
                                            var rotation = ((SelectFieldFromClass(item4.Value as IReadOnlyDictionary<HkField, IHkObject>, "rotation") as IHkObject).Value as IReadOnlyList<IHkObject>).Cast<HkSingle>().ToList();
                                            var scale = ((SelectFieldFromClass(item4.Value as IReadOnlyDictionary<HkField, IHkObject>, "scale") as IHkObject).Value as IReadOnlyList<IHkObject>).Cast<HkSingle>().ToList();
                                            ...
                                            if (refFrameClass != null)
                                            {
                                                ...
                                                foreach (var k in refFrameClass.Value as IReadOnlyDictionary<HkField, IHkObject>)
                                                {
                                                    ...
                                                    else if (k.Key.Name == "referenceFrameSamples")
                                                    {
                                                        ...
                                                        foreach (var item7 in (k.Value.Value as IReadOnlyList<IHkObject>))
                                                        {
                                                            var refSample = (item7.Value as IReadOnlyList<IHkObject>).Select(x => ((HkSingle)x).Value).ToList();
                            ...
                            else if (anim.Type.Name == "hkaInterleavedUncompressedAnimation")
                            {
                                ...
                                foreach (var kvp in anim.Value as IReadOnlyDictionary<HkField, IHkObject>)
                                {
                                            ...
                                            if (refFrameClass != null)
                                            {
                                                ...
                                                foreach (var k in refFrameClass.Value as IReadOnlyDictionary<HkField, IHkObject>)
                                                {
                                                    ...
                                                    else if (k.Key.Name == "referenceFrameSamples")
                                                    {
                                                        var arrData = new HKArrayData<HKVector4>();
                                                        arrData.Elements = new List<HKVector4>();
                                                        foreach (var item8 in (k.Value.Value as IReadOnlyList<IHkObject>))
                                                        {
                                                            var refSample = (item8.Value as IReadOnlyList<IHkObject>).Select(x => ((HkSingle)x).Value).ToList();
                                                            arrData.Elements.Add(new HKVector4(new System.Numerics.Vector4(refSample[0], refSample[1], refSample[2], refSample[3])));
                                                        }
                                                        hkaDefaultAnimatedReferenceFrame.ReferenceFrameSamples = new HKArray<HKVector4>(arrData);
                                                    }
                                    ...
                                    else if (kvp.Key.Name == "transforms")
                                    {
                                        var arrData = new HKArrayData<Transform>();
                                        arrData.Elements = new List<Transform>();
                                        foreach (var item9 in kvp.Value.Value as IReadOnlyList<IHkObject>)
                                        {
                                            var obj2 = item9 as HkClass;
                                            var tr = new Transform();
                                            foreach (var k in obj2.Value as IReadOnlyDictionary<HkField, IHkObject>)
                                            {
                        ...
                        {
                            foreach (var item5 in animations.Value as IReadOnlyList<IHkObject>)
                            {
                                var anim = item5.Value as HkClass;
                                DoAnim(anim);
                            }
                        }
                        ...
                        {
                            foreach (var item6 in bindings.Value as IReadOnlyList<IHkObject>)
                            {
                                var obj3 = item6.Value as HkClass;
                                ...
                                foreach (var kvp in obj3.Value as IReadOnlyDictionary<HkField, IHkObject>)
                                {
                    ...
                    foreach (IHkObject item in (SelectFieldFromClass(obj.Value as IReadOnlyDictionary<HkField, IHkObject>, "namedVariants") as HkArray).Value)
