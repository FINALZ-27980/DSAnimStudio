...
namespace HKX2
{
    public class PackFileSerializer
    {
        ...
        private int _currSerializationQueue;
        ...
        public void Serialize(IHavokObject rootObject, BinaryWriterEx bw)
        {
            ...
            HKXClassName hKXClassName = new HKXClassName();
            hKXClassName.ClassName = "hkClass";
            hKXClassName.Signature = 0x33D42383;
            hKXClassName.Write(classbw);
            hKXClassName.ClassName = "hkClassMember";
            hKXClassName.Signature = 0xB0EFA719;
            hKXClassName.Write(classbw);
            hKXClassName.ClassName = "hkClassEnum";
            hKXClassName.Signature = 0x8A3609CF;
            hKXClassName.Write(classbw);
            hKXClassName.ClassName = "hkClassEnumItem";
            hKXClassName.Signature = 0xCe6F8A6C;
            hKXClassName.Write(classbw);
            ...
            while (_serializationQueues.Count > 1 || _serializationQueues[0].Count > 0)
            {
                ...
                if (_pendingVirtuals.Contains(obj))
                {
                    ...
                    if (!_virtualTableLookup.ContainsKey(classname))
                    {
                        ...
                        HKXClassName hKXClassName2 = new HKXClassName();
                        hKXClassName2.ClassName = classname;
                        hKXClassName2.Signature = obj.Signature;
                        uint offset = (uint)classbw.Position;
                        hKXClassName2.Write(classbw);
            ...
            HKXSection hKXSection = new HKXSection();
            hKXSection.SectionID = 2;
            hKXSection.SectionTag = "__data__";
            hKXSection.SectionData = datams.ToArray();
            hKXSection.VirtualFixups = _virtualFixups;
            hKXSection.GlobalFixups = _globalFixups.OrderBy((x) => x.Src).ToList();
            hKXSection.LocalFixups = _localFixups.OrderBy((x) => x.Dst).ToList();
            hKXSection.WriteHeader(bw, HKX.HKXVariation.HKXDS3);

            classNames.WriteData(bw);
            types.WriteData(bw);
            hKXSection.WriteData(bw);
