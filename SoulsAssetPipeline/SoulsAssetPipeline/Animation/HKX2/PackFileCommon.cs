...
namespace HKX2
{
    ...
    internal class HKXSection
    {
        ...
        public void WriteData(BinaryWriterEx bw)
        {
            ...
            foreach (var localFixup in LocalFixups)
            {
                localFixup.Write(bw);
            }
            while ((bw.Position % 16) != 0L)
            ...
            foreach (var globalFixup in GlobalFixups)
            {
                globalFixup.Write(bw);
            }
            while ((bw.Position % 16) != 0L)
            {
                bw.WriteByte(0xFF); // 16 byte align
            ...
            foreach (var virtualFixup in VirtualFixups)
            {
                virtualFixup.Write(bw);
            }
            while ((bw.Position % 16) != 0L)
            {
                bw.WriteByte(0xFF); // 16 byte align
        ...
        internal HKXClassNames ReadClassnames(PackFileDeserializer hkx)
        {
            ...
            var hKXClassNames = new HKXClassNames();
            hKXClassNames.Read(hkx, this, br, HKX.HKXVariation.HKXDS3);
            return hKXClassNames;
        }
    }
}
