...
namespace DSAnimStudio.ImguiOSD
{
    public abstract partial class Window
    {
        public class WorkspaceConfig : Window
        {
            ...
            public override string Title => $"实体实用 - {GetEntityDispText(EntityType, EntityName)}";
            ...
            protected override void BuildContents()
            {
                ...
                if (entityType == TaeViewportInteractor.TaeEntityType.NPC)
                {
                    ...
                    if (mdl != null)
                    {
                        if (ImGui.TreeNode("加载NpcParam预设（主要是快速切换同模组敌人的不同mask显示）"))
                        {
                ...
                {
                    OSD.WindowEditPlayerEquip.IsOpen = MenuBar.CheckboxBig("打开玩家装备菜单", OSD.WindowEditPlayerEquip.IsOpen);
                    ...
                    if (Tae?.Graph?.ViewportInteractor?.EventSim != null)
                    {
                        ...
                        ImGui.ListBox("行为组读取对象（义手和武技分别是左右手武器，切换这个可以正常看武技或忍具伤害判定）", ref currentHitViewSourceIndex, BehaviorHitboxSrcEnum_Names, BehaviorHitboxSrcEnum_Names.Length);
                ...
                ImguiOSD.Tools.FancyComboBox("预览窗口信息显示类型", ref curIndex, viewportStatusTypes);
                if (curIndex < 0)
                    curIndex = 1;
