...
namespace DSAnimStudio.ImguiOSD
{
    public abstract partial class Window
    {
        ...
        {
            public override string Title => "模型，骨骼，dummy点，镜头设置";
            ...
            protected override void BuildContents()
            {
                ...
                lock (Scene._lock_ModelLoad_Draw)
                {
                    ...
                    if (ImGui.TreeNode("模型设置"))
                    {
                        GFX.HideFLVERs = !MenuBar.Checkbox("显示模型", !GFX.HideFLVERs);
                    ...
                    if (ImGui.TreeNode("骨骼和Dummy显示设置"))
                    {
                        lock (DBG._lock_DebugDrawEnablers)
                        {
                            ...
                            DBG.CategoryEnableDraw[DebugPrimitives.DbgPrimCategory.DummyPoly] =
                                MenuBar.Checkbox("显示Dummy点（全部，调整单个显示需要到toolbox>DummyPoly）",
                                DBG.CategoryEnableDraw[DebugPrimitives.DbgPrimCategory.DummyPoly],
                            ...
                            DBG.CategoryEnableNameDraw[DebugPrimitives.DbgPrimCategory.DummyPoly] =
                                MenuBar.Checkbox("显示Dummy点ID（全部）",
                                DBG.CategoryEnableNameDraw[DebugPrimitives.DbgPrimCategory.DummyPoly],
                                enabled: true, shortcut: "(This Color)", shortcutColor: Main.Colors.ColorHelperDummyPoly);

                            NewDummyPolyManager.ShowGlobalIDOffset = MenuBar.Checkbox("武器Dummy点ID显示通用前缀\n（右手10左手20）", NewDummyPolyManager.ShowGlobalIDOffset);
                            ImGui.NewLine();
                            DBG.CategoryEnableDraw[DebugPrimitives.DbgPrimCategory.SoundEvent] =
                                MenuBar.Checkbox("显示声音ID（DSSI里那种ID）",
                                DBG.CategoryEnableDraw[DebugPrimitives.DbgPrimCategory.SoundEvent],
                                enabled: true, shortcut: "(This Color)", shortcutColor: Main.Colors.ColorHelperSoundEvent);

                            RemoManager.EnableDummyPrims =
                                MenuBar.Checkbox("过场动画Dummy点",
                                RemoManager.EnableDummyPrims,
                        ...
                        Tae.Config.DbgPrimXRay = MenuBar.Checkbox("勾选这个才能看到骨骼线条", Tae.Config.DbgPrimXRay);
