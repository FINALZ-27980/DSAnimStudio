...
namespace DSAnimStudio
{
    ...
    {
        public const string VERSION = "4.9.9 [Build 4999]";
        ...
        private static void UpdateLazyDispatch()
        {
            ...
            lock (_lock_LazyDispatch)
            {
                ...
                {
                    var lazyDispatchAction = LazyDispatchActionQueue.Peek();

                    lazyDispatchAction.GhettoTimer--;

                    if (lazyDispatchAction.GhettoTimer <= 0)
                    {
        ...
        public static void DrawSplash(bool clear)
        {
            ...
            float num = (float)GFX.Device.Viewport.Bounds.Width / (float)GFX.Device.Viewport.Bounds.Height;
        ...
        public static void UpdateSplash(float deltaTime)
        {
            if (SplashState == SplashScreenStates.FadeIn)
            {
                ...
                SplashAlpha = SplashTimer / SplashFadeDuration;
        ...
        public static string ImGuiFontName => $@"{Directory}\Content\Fonts\SDK_SC_WEB.ttf";
        public static float ImGuiFontGlyphMinAdvanceX = 5.0f;
        ...
        private static void CheckConfigFilePath()
        {
            ...
            {
                ConfigFilePath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), ConfigFileShortName);
            }
        }
        ...
        private bool prevFrameWasLoadingTaskRunning;
        ...
        public bool IsLoadingTaskRunning;
