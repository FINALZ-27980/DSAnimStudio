    ...
    public class TaeEventSimulationEnvironment
    {
        ...
        private void InitAllEntries()
        {
            entries = new Dictionary<string, EventSimEntry>
            {
                ...
                {
                    "EventSimWeaponStyle",
                    new EventSimEntry("启用武器风格变化（单双持之类）", true, "SetWeaponStyle")
                    {
                ...
                {
                    "EventSimTracking",
                    new EventSimEntry("启用A/D旋转视角快捷键", true)
                    {
                        SimulationFrameChangeDisabledAction = (entry, evBoxes, time) =>
                        {
                ...
                //},
                {
                    ...
                    new EventSimEntry("启用Blend词条模拟", true)
                    {
                        SimulationFrameChangeDisabledAction = (entry, evBoxes, time) =>
                        {
...

                {
                    ...
                    new EventSimEntry("启用连招预览里的Blend词条模拟", true)
                    {
                        SimulationFrameChangeDisabledAction = (entry, evBoxes, time) =>
                        {
                            if (Graph.ViewportInteractor.CurrentComboIndex < 0)
...

                {
                    ...
                    new EventSimEntry("启用607面部表情词条模拟", true)
                    {
                        SimulationFrameChangeDisabledAction = (entry, evBoxes, time) =>
                        {
...

                {
                    ...
                    new EventSimEntry("启用715 override词条武器位置变化显示", true)
                    {
                        SimulationFrameChangeDisabledAction = (entry, evBoxes, time) =>
                        {
...

                {
                    ...
                    new EventSimEntry("启用攻击判定显示", isEnabledByDefault: true,
...

                {
                    ...
                    new EventSimEntry("启用黑魂三603变速词条（其实只狼也有这个词条，但是使这个词条生效需要插件hoodiepatch.dll，作者没有更新对只狼的支持，悲）", isEnabledByDefault: false, "DebugAnimSpeed")
                    {
                        SimulationFrameChangePreBoxesAction = (entry, evBoxes, time) =>
                        {
                            Graph.PlaybackCursor.ModPlaybackSpeed_Event603 = 1;
...

                {
                    ...
                    new EventSimEntry("启用法环608变速词条", isEnabledByDefault: true, "AnimSpeedGradient"
#if NIGHTFALL
                        , "NF_SpeedGradient"
#endif
                    )
...

                {
                    ...
                    new EventSimEntry("启用子弹产生显示", isEnabledByDefault: true,
                ...
                {
                    "EventSimSFXSpawns",
                    new EventSimEntry("启用特效产生显示（4.9.3在播放一些动作遇到视觉特效词条时概率报错，原因不明，取消掉这项就行）", isEnabledByDefault: true,
                        "N/A")
                    {
                        AllowDuringRemo = true,
                        NewAnimSelectedAction = (entry, evBoxes) =>
                ...
                {
                    "EventSimOpacity",
                    new EventSimEntry("启用透明度变化", isEnabledByDefault: true,
                ...
                {
                    "EventSimDrawMasks",
                    new EventSimEntry("启用mask变化", isEnabledByDefault: true,
